...
| `Error Unit` | `"%"` で名目値に対する割合。長さ単位（`m` / `cm` / `mm`）なら絶対値として指定し、書き込み先のフィールド単位へ換算される。それ以外の文字列は書き込み先の単位そのままの絶対値として扱う |
| `Error 3sigma Low` | 名目値より小さい側の 3σ 値 |
| `Error 3sigma High` | 名目値より大きい側の 3σ 値 |

`Wind` のみ `Wind Files Zip Path` を使用（σ 値なし）。

**長さ単位の換算について**: 同じパラメータでも書き込み先の単位はモードで変わる。`XCP` / `XCG` の
定数フィールドは `... [mm]`（ソルバが 1e3 で割る）だが、対応するテーブルファイルは m
（ソルバはそのまま補間する）。`"Error Unit": "m"` はどちらの場合も正しく換算されるので、
指定側でモードを意識する必要はない。
...
#### ファイルモードのパラメータに誤差を与える場合

`param_rocket.json` / `param_engine.json` で `Enable * File = true` になっているパラメータは、
ソルバが定数フィールドを**一切読まない**（`rocket_factory.cpp` が if/else）。したがって誤差は
テーブル自体に適用され、ケースごとのテーブル CSV が `cases/` に書き出されて、そのケースの
config がそれを指す。適用のしかたは 2 通りで、`Error Unit` の種類がこれに対応している：

| 適用 | 対象パラメータ | 必要な `Error Unit` |
|---|---|---|
| 倍率（テーブル全体にケースごとの倍率を掛ける） | `CA`, `CNa`, `Cld`, `Clp`, `Cmq`, `Cnr`, `MOI`, `Thrust`, `POI Ixy/Ixz/Iyz` | `"%"` |
| オフセット（テーブル全体にケースごとの絶対量を加える） | `XCP`, `XCG` | 長さ単位（`m` / `cm` / `mm`） |

`XCP` / `XCG` がオフセットなのは、誤差指定が物理的な距離だからである。倍率にすると
CP/CG が末端から遠いマッハ域ほど大きく動き、指定した「±x m」と意味が合わなくなる。

組み合わせが誤っている場合（倍率対象に絶対単位、オフセット対象に `"%"` 等）、モンテカルロは
**ケース生成前に例外で停止する**。オフセット対象の名目値は 0 なので `"%"` 指定は「分散ゼロ」を
意味し、以前は完走してしまって気付けなかった。ファイルモードに未対応のパラメータへ誤差を
指定した場合も同様に停止する。
...
| `Variation Unit` | `%` または物理単位文字列。長さ単位（`m` / `cm` / `mm`）は書き込み先のフィールド単位へ換算される（モンテカルロの `Error Unit` と同じ規則） |
| `Variations` | 名目値からの変化量リスト（正負両側を推奨） |
| `Reference Variations` | `two_point` 時の差分計算に使う `[lo, hi]`（省略時は Variations の最小/最大） |

ファイルモードのパラメータもモンテカルロと同じ扱いで、ケースごとのテーブル CSV を書き出す
（倍率対象は `%`、`XCP` はオフセットなので長さ単位が必要）。単位の種類が合わない場合は例外で停止する。
