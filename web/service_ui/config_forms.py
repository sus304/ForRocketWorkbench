"""Typed parameter forms for the server-side project editor (docs/ui_refresh_design.md §4).

Faithful port of the legacy calculate.py form builders (review N-4), adapted to the compute
service model: builders take a plain config dict and return a ``collect()`` closure that reads the
current widget values back into a config dict. The editor page assembles every file's collect()
into one etag-guarded save (service/projects.write_config), so — unlike the legacy per-file save —
there is no direct filesystem access here.

The pure, view-free logic (list parsing, case-count estimates, sensitivity validation) lives as
module-level functions so it can be unit-tested without a NiceGUI client; the ``_build_*`` bodies
are the NiceGUI page code, verified via manual/e2e checks like the rest of web.service_ui.
"""
...
# ── Registries (mirror the runner's supported parameters) ────────────────────────
...
# (parameter name, default Variation Unit). Mirrors the runner's supported scalar sensitivity
# parameters (runner_montecarlo._SCALAR_PARAM_REGISTRY) plus Thrust / CA. Keep in sync with
# runner_tool/runner_sensitivity.py.
_SENS_PARAM_DEFS = [
    ('Thrust',                        '%'),
    ('CA',                            '%'),
...
# ── Pure logic (unit-tested; no NiceGUI) ─────────────────────────────────────────

def parse_num_list(text: str) -> list:
    """Parse a comma/semicolon-separated numeric list, keeping ints int and floats float."""
...
def opts_with(value, base: list) -> list:
    """Return base options with `value` prepended if absent.

    ui.select raises ValueError when its initial value is outside `options`, so any custom
    value loaded from a config must be merged in first.
    """
    opts = list(base)
...
def area_case_count(spd_lo, spd_hi, spd_step, dir_lo, dir_hi, dir_step) -> int:
    """Number of Area cases for the given wind speed/direction sweep (matches runner_area)."""
    ns = max(1, round((spd_hi - spd_lo) / spd_step) + 1)
    nd = max(1, round((dir_hi - dir_lo) / dir_step) + 1)
    return ns * nd


def sensitivity_case_count(params: List[dict]) -> int:
    """Total sensitivity cases = 1 nominal + one per variation value across all parameters."""
    return 1 + sum(len(p.get('Variations') or []) for p in params)


def validate_sensitivity(params: List[dict], thrust_fm=False, ca_fm=False, poi_fm=False) -> List[str]:
    """Return human-readable warnings for a sensitivity parameter set (empty = clean).

    Mirrors runner_sensitivity's constraints: ≥2 variations per parameter, known parameter names,
    coupled parameters need Effects, and file-mode Thrust/CA/POI accept only a "%" unit.
    """
    msgs: List[str] = []
    names = [p.get('Name') or '(unnamed)' for p in params]
    for p in params:
        ...
        if unit != '%':
            for pn in pset:
                if (pn == 'Thrust' and thrust_fm) or (pn == 'CA' and ca_fm) \
                    ...
                    msgs.append(f'"{nm}": {pn} is in file mode → Unit must be "%" (got "{unit}")')
...
def _build_solver_form(data: dict, container) -> Callable[[], dict]:
...
def _build_stage_form(data: dict, container) -> Callable[[], dict]:
    with container:
        with ui.card().classes('w-full'):
    ...
    def collect() -> dict:
        r = dict(data)
        r['Rocket Configuration File Path'] = rocket.value
        r['Engine Configuration File Path'] = engine.value
        r['Sequence of Event File Path'] = soe.value
        return r

    return collect


def _build_rocket_form(data: dict, container) -> Callable[[], dict]:
    mass = data.get('Mass', {})
    gj = data.get('Gas Jet', {})
    pa = data.get('Program Attitude', {})
    xcg_c = data.get('Constant X-C.G.', {})
    # v4.3.0+ keeps the lateral CG offset in a dedicated "C.G. Offset" block; fall back to the
    # legacy nested keys under "Constant X-C.G." for old files.
    ...
    with container:
        ...
        with ui.card().classes('w-full'):
            ...
            # Lateral CG offset is an inert-value constant (ForRocket derives the full-vehicle
            # time history internally) — editable in both X-C.G. modes.
            with ui.grid(columns=2).classes('w-full'):
...
def _build_engine_form(data: dict, container) -> Callable[[], dict]:
...
def _build_soe_form(data: dict, container) -> Callable[[], dict]:
    ...
    with container:
        ...
        # Adaptive Solver Tolerance (v4.4.0+): 適応ステップ積分器の許容誤差。Abs は姿勢(小振幅状態)
        # 精度、Rel は ECI 位置精度と計算速度を支配。既定 Abs=1e-8/Rel=1e-6 は高速化と姿勢ロバスト性
        # のバランス点。
        with ui.card().classes('w-full'):
...
def _build_area_form(data: dict, container) -> Callable[[], dict]:
    ...
    with container:
        ...
        def _upd_cases():
            try:
                case_lbl.set_text(
                    f'Estimated cases: {area_case_count(spd_lo.value, spd_hi.value, spd_step.value, dir_lo.value, dir_hi.value, dir_step.value)}')
            except Exception:
                pass
...
def _build_montecarlo_form(data: dict, container) -> Callable[[], dict]:
    ...
    def collect() -> dict:
        ...
        r = dict(data)
        r['MonteCarlo Case Count'] = int(case_count.value or 300)
        r['Output All Case Logs'] = bool(output_all_logs.value)
        r['Error Parameters'] = ep_out
        return r

    return collect


def _build_sensitivity_form(data: dict, container, rocket: Optional[dict] = None,
                            engine: Optional[dict] = None) -> Callable[[], dict]:
    ...
    # Working copy — edits are buffered here and only written back on collect().
    ...
    # (runner_sensitivity._get_nominal raises otherwise). The sibling configs are the ones the
    # editor already loaded for this project.
    rocket = rocket or {}
    engine = engine or {}
    ...
    def _sync():
        for row, p in zip(param_rows, params_data):
            ...
            p['Variations'] = parse_num_list(row['variations'].value)
    ...
    def _update_total():
        total_lbl.set_text(f'Estimated cases: 1 (nominal) + {sensitivity_case_count(params_data) - 1} '
                           f'= {sensitivity_case_count(params_data)}')

    def _validate():
        msgs = validate_sensitivity(params_data, thrust_fm, ca_fm, poi_fm)
        if msgs:
    ...
    def _init_row_ui(row):
        ...
        vals = parse_num_list(row['variations'].value)
        ...
        for key, default in (('ref_lo', ref_vals[0]), ('ref_hi', ref_vals[-1])):
            sel = row[key]
                ...
                with ui.card().classes('w-full q-pa-sm'):
                    ...
                    with ui.row().classes('items-center no-wrap w-full q-gutter-sm'):
                        if coupled:
                            ...
                            row['name'] = ui.select(
                                options=opts_with(cur, _SENS_PARAM_NAMES), value=cur,
                        ...
                        row['unit'] = ui.select(options=opts_with(cu, _SENS_UNITS), value=cu,
                        ...
                        with ui.card().classes('w-full q-pa-xs wb-subpanel'):
                            ...
                            for j, eff in enumerate(p.get('Effects') or []):
                                with ui.row().classes('items-center no-wrap q-gutter-sm'):
                                    ...
                                    esel = ui.select(options=opts_with(ecur, _SENS_PARAM_NAMES),
    ...
    def collect() -> dict:
        ...
        for p in params_data:
            ...
            result.append(entry)
        r = dict(data)
        r['Sensitivity Calculation'] = {'Method': method_sel.value}
        r['Sensitivity Parameters'] = result
        return r
...
def has_form(fname: str) -> bool:
    return fname in FORM_BUILDERS


def build_form(fname: str, data: dict, container, siblings: Optional[dict] = None) -> Optional[Callable[[], dict]]:
    """Render the typed form for `fname` inside `container` and return its collect() closure, or
    None if there is no typed form. `siblings` is the project's other loaded configs (used by the
    sensitivity form to detect Thrust/CA/POI file mode)."""
    builder = FORM_BUILDERS.get(fname)
    if builder is None:
        return None
    if builder is _build_sensitivity_form:
        siblings = siblings or {}
        return builder(data or {}, container,
                       rocket=siblings.get('param_rocket.json'),
                       engine=siblings.get('param_engine.json'))
    return builder(data or {}, container)
